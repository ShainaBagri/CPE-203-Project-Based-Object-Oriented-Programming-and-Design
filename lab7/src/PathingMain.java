...
public class PathingMain extends PApplet
{
   ...
   private Point nextPoint(Point pos, GridValues[][] grid) {
	   //check right
   ...
   private boolean depthFirstSearch(Point pos, GridValues[][] grid, List<Point> path) {
	   ...
	   if(grid[pos.y][pos.x]!=GridValues.GOAL) {
		   if(grid[pos.y][pos.x]!=GridValues.SEARCHED) {
			   path.add(pos);
			   grid[pos.y][pos.x] = GridValues.SEARCHED;
		   }
		   
		   Point newPos = nextPoint(pos, grid);
		   if(newPos==null) {
			   path.remove(path.size()-1);
			   newPos = path.get(path.size()-1);
		   }
		   depthFirstSearch(newPos, grid, path);
	   }
	   return true;
   }
